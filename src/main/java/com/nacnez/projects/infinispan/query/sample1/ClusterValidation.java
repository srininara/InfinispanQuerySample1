...
public class ClusterValidation {
	 ...
	 * 
	 * @param cacheManager
	 *            the {@link EmbeddedCacheManager} to use to check if the
	 *            cluster has formed
	 * @param nodeId
	...
	public static boolean waitForClusterToForm(
		...
		Cache<String,String> cache = cacheManager.getCache("Dummy");
		return new ClusterValidation(cache, nodeId,
...
//	private final Cache<Object, Object> cache;
	...
	private ClusterValidation(Cache<String, String> cache, int nodeId,
			int clusterSize) {
		this.cache = cache;
	...
	private int checkReplicationSeveralTimes() {
		...
		for (int i = 0; i < REPLICATION_TRY_COUNT; i++) {
			...
			tryToPut();
			// Wait for a bit to allow replication and wait for the other node
			// to
			// start
			try {
			...
			if (replCount == clusterSize - 1) {
				// The cluster has formed if there are n-1 replicas in the
				// cluster
				log.info("Cluster formed successfully!");
				// We try to put one last time to ensure that *our* key is now
				// in
...
//				Collection<Person> persons = DataCreator.createData(1);
